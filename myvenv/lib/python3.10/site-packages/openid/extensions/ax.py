...
class FetchRequest(AXMessage):
    ...
    def add(self, attribute):
        ...
        if attribute.type_uri in self.requested_attributes:
            raise KeyError(
                "The attribute %r has already been requested" % (attribute.type_uri,)
            )
                ...
                if count_s:
                    try:
                        ...
                        if count_s != UNLIMITED_VALUES:
                            raise AXError(
                                "Invalid count value for %r: %r" % (count_key, count_s,)
                            )
                        count = count_s
                else:
                    count = 1
        ...
        if self.request is not None:
            ...
            for type_uri in self.data:
                if type_uri not in self.request:
                    raise KeyError(
                        "Response attribute not present in request: %r" % (type_uri,)
                    )
...
class StoreResponse(AXMessage):
    ...
    def __init__(self, succeeded=True, error_message=None):
        ...
        if succeeded and error_message is not None:
            raise AXError(
                "An error message may only be included in a " "failing fetch response"
            )
        if succeeded:
